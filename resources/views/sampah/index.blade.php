            ...
            <div class="modal-dialog modal-fullscreen modal-dialog-scrollable">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="/usages/sampah/cetak" method="post" id="FormCetakBuktiTagihan" target="_blank">
                            @csrf
        ...
        <script>
            ...
            $(document).on('click', '#Registerpemakaian', function(e) {
                ...
                if (bulanTerpilih && caterId) {
                    window.location.href = '/usages/create/sampah?bulan=' + bulanTerpilih + '&cater_id=' + caterId;
                } else {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Peringatan',
                ...
                if (cater != '') {
                    if (table == '') {
                        table = $('#TbPemakain').DataTable({
                            ...
                            "serverSide": true,
                            "ajax": {
                                "url": "/usages/sampah?bulan=" + bulan + "&tahun=" + tahun + "&cater=" + cater,
                                "type": "GET"
                            },
                            "language": {
            ...
            $(document).on('click', '#DetailCetakBuktiTagihanSampah', function(e) {
                    ...
                    items.forEach(item => {
                        tbTagihan.find('tbody').append(`
                            <tr>
                                ...
                                <td align="center">${new Intl.NumberFormat('id-ID', {
                                                        minimumFractionDigits: 2,
                                                        maximumFractionDigits: 2
                                                    }).format(item.installation?.abodemen ?? 0)}
                                </td>
            ...
            $(document).on('click', '#BtnCetak', function(e) {
                ...
                function processChunk(index) {
                    if (index >= chunks.length) {
                        ...
                        html += '<body style="margin:0;font-family:Arial,sans-serif;background:linear-gradient(135deg,#0077b6,#00b4d8);min-height:100vh;display:flex;justify-content:center;align-items:center;padding:40px">';
                        html += '<div style="background:#fff;border-radius:12px;box-shadow:0 8px 32px rgba(0,0,0,0.15);max-width:800px;width:100%">';
                        html += '<div style="background:#0d6efd;padding:36px 40px;border-radius:12px 12px 0 0;text-align:center">';
                        html += '<div style="width:64px;height:64px;background:rgba(255,255,255,0.2);border-radius:50%;display:inline-flex;align-items:center;justify-content:center;margin-bottom:14px"><span style="font-size:32px;color:#fff">&#10003;</span></div>';
                        html += '<h2 style="color:#fff;margin:0 0 8px;font-size:28px">Struk Berhasil Dibuat!</h2>';
                        html += '<p style="color:rgba(255,255,255,0.85);margin:0;font-size:17px">' + allIds.length + ' struk retribusi sampah siap dicetak</p>';
                        html += '</div>';
                        html += '<div style="padding:32px 40px">';
                        html += '<div style="background:#fff3cd;border-left:5px solid #ffc107;padding:18px 24px;border-radius:6px;margin-bottom:28px">';
                        ...
                        html += '</div>';
                        html += '<p style="color:#495057;font-size:16px;margin:0 0 20px;font-weight:bold">Pilih bagian yang ingin dibuka:</p>';
                        results.forEach(function(r, i) {
                            html += '<a href="' + r.url + '" target="_blank" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;padding:20px 24px;background:#f8f9fa;border:1px solid #dee2e6;border-radius:8px;text-decoration:none;color:#212529">';
                            html += '<div><strong style="color:#0d6efd;font-size:18px">Bagian ' + (i + 1) + '</strong><br><span style="color:#6c757d;font-size:15px">' + r.count + ' data pelanggan</span></div>';
                            html += '<span style="background:#0d6efd;color:#fff;padding:10px 28px;border-radius:6px;font-size:16px;font-weight:bold">Buka & Cetak</span>';
                            html += '</a>';
                        });
                        html += '<p style="color:#adb5bd;font-size:14px;margin:24px 0 0;text-align:center;border-top:1px solid #e9ecef;padding-top:20px">Tip: Setelah struk terbuka, tekan Ctrl+P untuk langsung mencetak.</p>';
    ...
    `);
        ...
        <script>
            ...
            $(document).on('click', '.Hapus_pemakaian', function(e) {
                ...
                var actionUrl = '/usages/sampah/' + hapus_pemakaian; // URL endpoint untuk proses hapus
